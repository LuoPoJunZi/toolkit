#!/bin/bash
# Vendored upstream snapshot derived from kejilion/sh (Apache-2.0).
# Local adaptations:
# - disabled upstream telemetry
# - disabled self-install side effects in library mode
# - added library-mode guards for safe sourcing in LuoPo VPS Toolkit
sh_v="4.4.10"
...
if [ -z "${KEJILION_LIBRARY_MODE:-}" ]; then
canshu_v6
CheckFirstRun_true
yinsiyuanquan2
...
kejilion() {
			if [ -n "${KEJILION_LIBRARY_MODE:-}" ]; then
				exit 0
			fi
			cd ~
			kejilion_sh
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装LuoPo脚本           ${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}清理系统"
